...
autoGameLoop board = do
    printBoard board
    threadDelay 400000  -- Pausa de 400 ms (ajustable para velocidad)
...
interactiveSetup board cursor = do
    ...
    hFlush stdout
    key <- getLine
    case fmap toLower (if not (null key) then Just (head key) else Nothing) of
        Just 'v' -> do
            let (x, y) = cursor
        ...
        Just 'i' -> return board
        Just k -> do
            let newCursor = moveCursor cursor k (length board) (length (head board))
            interactiveSetup board newCursor
        Nothing -> interactiveSetup board cursor

-- Selecciona un patrón por defecto o permite personalización
choosePatternOrCustomize :: IO Board
choosePatternOrCustomize = do
    putStrLn "Elige un patrón predefinido o personaliza el tablero:"
    mapM_ (\(i, (name, _)) -> putStrLn (show i ++ ". " ++ name)) (zip [1..] patterns)
    putStrLn "0. Personalizar"
    putStr "Tu elección: "
    hFlush stdout
    choice <- readLn
    if choice >= 1 && choice <= length patterns
        then let (rows, cols, positions) = snd (patterns !! (choice - 1))
             in return $ initializeBoard (emptyBoard rows cols) positions
        else do
...
main = do
    ...
    hideCursor
    board <- choosePatternOrCustomize
    showCursor
